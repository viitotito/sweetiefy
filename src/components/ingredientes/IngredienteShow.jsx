...
const IngredienteShow = () => {
  ...
  useEffect(() => {
    const fetchIngrediente = async () => {
      try {
        const res = await authFetch(`${API_URL}/api/ingredientes/${id}`);
  ...
  return (
    <div className="card p-4 shadow-sm mx-auto" style={{ maxWidth: "600px" }}>
      <div className="mb-3">
        ...
        <input type="text" className="form-control" value={ingrediente.nome} disabled />
      </div>

      <div className="mb-3">
        <label className="form-label fw-bold">Preço (R$)</label>
        <input type="text" className="form-control" value={Number(ingrediente.preco || 0).toFixed(2)} disabled />
      </div>

      <div className="mb-3">
        <label className="form-label fw-bold">Métrica</label>
        <input type="text" className="form-control" value={ingrediente.metrica} disabled />
      </div>
      ...
      <div className="d-flex justify-content-center">
        <button className="btn btn-outline-secondary" onClick={() => navigate("/ingredientes")}>
          Voltar
        </button>
      </div>
    </div>
